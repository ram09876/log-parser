...
const {Utils, GetUAsUtils} = require('../utils')
...
class GetCodesCommand extends Command {
  async run() {
    ...
    GetUAsUtils.CreateSummary(fromDate, toDate, max, Utils.ValidateInteger(flags.code));
...
GetCodesCommand.flags = {
  ...
  code: flags.string({char: 'c', description: 'status code for urls'}),
}
