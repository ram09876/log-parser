               ...
               allKeys.forEach(((element) => {
                   ...
                   element.forEach((innerElement) => {
                    ...
                    logEntryObjList.push(entryObj)
                      ...
                      if(lastLineReader){
                        var filteredEntries = logEntryObjList.filter((entry) => entry.elb_status_code > 400)
                        var groupStatusCodes = groupBy(filteredEntries, "request")
                        ...
                        const table = new Table({
                            head: [`Top ${max} Error prone urls`, 'Count']
                          , colWidths: [100, 15]
                        });
                        ...
                        console.log('\x1b[36m%s\x1b[0m', `Below Table shows top ${max} Error urls given timeframe`);
                        console.log(table.toString());



                        var groupStatusCodes = groupBy(filteredEntries, "elb_status_code")
                        groupStatusCodes = sortDesc(groupStatusCodes);
                        ...
                        var table1 = new Table({
                            head: [`Top ${max} Error codes`, 'Count']
                          , colWidths: [75, 15]
                        });
 
                        output.forEach((element) => {
                            
                            table1.push(Object.entries(element)[0])
                        })
                        console.log('\x1b[36m%s\x1b[0m', `Below Table shows top ${max} Error Status Codes for given timeframe`);
                        console.log(table1.toString());


                        
                        
                        // groupStatusCodes = groupBy(logEntryObjList, "request")
                        // groupStatusCodes = sortDesc(groupStatusCodes);
                        ...
                        //     head: [`Top ${max} popular urls`, 'Count']
                        //   , colWidths: [100, 15]
                        // });
 
                        // output.forEach((element) => {
                            
                        //     table2.push(Object.entries(element)[0])
                        // })
                        // console.log('\x1b[36m%s\x1b[0m', `Below Table shows top ${max} popular urls for given timeframe`);
